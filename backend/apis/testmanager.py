...
def searchBykey():
    ...
    if 'pickTime' in body and body['pickTime'] != '':
        if body['pickTime'][0] != body['pickTime'][1]:
            sql = sql + " AND R.updateDate >= '{}' and R.updateDate <= '{}' ".format(body['pickTime'][0], body['pickTime'][1])
            # sql = sql + " AND date(R.updateDate) between '{}' and '{}' ".format(body['pickTime'][0], body['pickTime'][1])
    ...
    with connection:
        ...
        with connection.cursor() as cursor:
            ...
            print('SELECT A.appId,R.* FROM request as R , apps as A '
                           'where R.appId = A.id AND R.isDel=0' + sql_list)
...
def uploadFile():
    ...
    print("validate ", file_form.validate())
    if file_form.validate():
        # 保存文件的相对路径
        save_path = os.path.join(os.path.abspath(os.path.dirname(__file__)).split('apis')[0], 'tmp/report_upload_file')
        # 获取文件
