...
# application.py
import sys
sys.path.append("/home/mi/flask_vue")
from flask import Blueprint
from dbutils.pooled_db import PooledDB
from backend.config import config, format
...
def searchBykey():
    ...
    with connection:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute('SELECT COUNT(*) as `count` FROM `apps` WHERE `status`=0')
            total = cursor.fetchall()

        # 执行查询
        with connection.cursor() as cursor:
            # 按照条件进行查询
            target_fields = ['id', 'appId', 'note', 'productId', 'developer', 'producer', 'tester',
                             'updateUser', 'updateDate']
            target_fields = ','.join(target_fields)
            cursor.execute('SELECT ' + target_fields + ' FROM apps WHERE `status`=0' + sql)
        ...
        if 'id' in body and body['id'] != '':
            ...
            with connection.cursor() as cursor:
                ...
                sql = "INSERT INTO `apps` (`appId`,`productId`,`note`,`tester`,`developer`,`producer`,`cCEmail`,`gitCode`" \
                      ",`wiki`,`more`,`creatorUser`,`updateUser`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                cursor.execute(sql, (body["appId"], body["productId"], body["note"], body["tester"], body["developer"], body['producer'], body["cCEmail"],
                                     body["gitCode"], body["wiki"], body["more"], body["creatorUser"], body["updateUser"]))
                # 提交执行保存新增数据
                connection.commit()
...
def getProduct():
    ...
    with connection.cursor() as cursor:
        # 查询产品信息表-按更新时间新旧排序
        sql = "SELECT id,productId FROM `apps` "
