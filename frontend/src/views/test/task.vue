<template>
  <div class="app-container">
    ...
    <div>
      <el-table v-loading="loading" :data="testData">
...
export default {
      ...
      pickerOptions: {
        shortcuts: [{
          ...
          onClick(picker) {
            ...
            console.log('pickTime start ', start.setTime(start.getTime() - 3600 * 1000 * 24 * 7))
  ...
  methods: {
    formatDate(row, column) {
      ...
      console.log('pickTime date ', moment(date).utcOffset(0).format('YYYY-MM-DD HH:mm'))
    ...
    productList() {
      apiAppsProduct().then(resp => {
        ...
        console.log('this.optsProduct', this.optsProduct)
      })
    },
    searchClick() {
      this.loading = true
      const body = {
        ...
        pickTime: [moment(this.search.pickTime[0]).utcOffset(0).format('YYYY-MM-DD HH:mm'), moment(this.search.pickTime[1]).utcOffset(0).format('YYYY-MM-DD HH:mm')],
